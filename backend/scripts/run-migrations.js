...
async function getExecutedMigrations() {
    try {
        const results = await (0, database_1.query)('SELECT migration_name FROM migrations ORDER BY migration_name');
...
async function markMigrationAsExecuted(migrationName) {
    await (0, database_1.query)('INSERT INTO migrations (migration_name) VALUES (?) ON DUPLICATE KEY UPDATE migration_name=migration_name', [migrationName]);
}
/**
    ...
    for (const statement of statements) {
        if (statement.trim()) {
            try {
                ...
                const isDropError = (error.code === 'ER_CANT_DROP_FIELD_OR_KEY' ||
                ...
                if (isDropError && (statement.toUpperCase().includes('DROP INDEX') ||
                ...
                const isExamRoomsError = (error.code === 'ER_BAD_FIELD_ERROR' ||
